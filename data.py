...
PIANO = (A0,As0,B0,C1,Cs1,D1,Ds1,E1,F1,Fs1,G1,Gs1,A1,As1,B1,C2,
...
do = [note for note in PIANO if note.name == 'C']
de = [note for note in PIANO if note.name == 'Cis']
re = [note for note in PIANO if note.name == 'D']
ri = [note for note in PIANO if note.name == 'Dis']
mi = [note for note in PIANO if note.name == 'E']
fa = [note for note in PIANO if note.name == 'F']
fi = [note for note in PIANO if note.name == 'Fis']
so = [note for note in PIANO if note.name == 'G']
sa = [note for note in PIANO if note.name == 'Gis']
la = [note for note in PIANO if note.name == 'A']
chi = [note for note in PIANO if note.name == 'Ais']
si = [note for note in PIANO if note.name == 'B']

c_major = Major_Scale(do)
c_major.start_with(C4,PIANO)

def check_doremi():
    if do != [note for note in PIANO if note.Hz % C1.Hz == 0]:
        return False
    if de != [note for note in PIANO if note.Hz % Cs1.Hz == 0]:
        return False
    if re != [note for note in PIANO if note.Hz % D1.Hz == 0]:
        return False
    if ri != [note for note in PIANO if note.Hz % Ds1.Hz == 0]:
        return False
    if mi != [note for note in PIANO if note.Hz % E1.Hz == 0]:
        return False
    if fa != [note for note in PIANO if note.Hz % F1.Hz == 0]:
        return False
    if fi != [note for note in PIANO if note.Hz % Fs1.Hz == 0]:
        return False
    if so != [note for note in PIANO if note.Hz % G1.Hz == 0]:
        return False
    if sa != [note for note in PIANO if note.Hz % Gs1.Hz == 0]:
        return False
    if la != [note for note in PIANO if note.Hz % A0.Hz == 0]:
        return False
    if chi != [note for note in PIANO if note.Hz % As0.Hz == 0]:
        return False
    if si != [note for note in PIANO if note.Hz % B0.Hz == 0]:
        return False
    return True

cd={'D':Chord('D','D',[0,4,7])}
nd={'D':Note('D',2)}

d_major = Key('D Major','Major_Scale','D')
the_rose = Music("THE ROSE","Amanda McBroom",d_major,4/4)
# the_rose_code_progression = [cd['D'],cd['D'],cd['A'].on(nd['C♯']),cd['G'].on(nd['B']),cd['A7'].sus4()]
the_rose_code_progression = ['D','D','AonC♯','GonB','A7']
